...
object PersistenceQueryDemo extends App {
  ...
  // select all persistence IDs
  val persistenceIds = readJournal.persistenceIds() // Source stream - picks up new ids as they're found from source
//  val currentPersistenceIds = readJournal.currentPersistenceIds() // finite set
  ...
  class MusicStoreEventAdapter extends WriteEventAdapter {
    override def toJournal(event: Any): Any = event match {
      case event @ PlaylistPurchased(_, songs) =>
        ...
        Tagged(event, genres) // wrapper for event that adds set of strings to it
      case event => event
    }
